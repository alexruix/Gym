...
DO $$ BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'peso_actual') THEN
        ALTER TABLE alumnos ADD COLUMN peso_actual DECIMAL(5,2);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'altura_cm') THEN
        ALTER TABLE alumnos ADD COLUMN altura_cm DECIMAL(5,2);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'objetivo_principal') THEN
        ALTER TABLE alumnos ADD COLUMN objetivo_principal TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'nivel_experiencia') THEN
        ALTER TABLE alumnos ADD COLUMN nivel_experiencia TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'profesion') THEN
        ALTER TABLE alumnos ADD COLUMN profesion TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'lesiones') THEN
        ALTER TABLE alumnos ADD COLUMN lesiones TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'perfil_completado') THEN
        ALTER TABLE alumnos ADD COLUMN perfil_completado BOOLEAN DEFAULT false;
    END IF;
...
-- Migración: Sincronización de columnas de perfil para Alumnos
DO $$ 
BEGIN
    -- 1. Columna Genero
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'genero') THEN
        ALTER TABLE public.alumnos ADD COLUMN genero TEXT;
    END IF;

    -- 2. Columna Frecuencia Semanal (para que el profesor la gestione)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'frecuencia_semanal') THEN
        ALTER TABLE public.alumnos ADD COLUMN frecuencia_semanal INTEGER DEFAULT 3;
    END IF;

    -- 3. Columna Perfil Completado (Flag técnica)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'perfil_completado') THEN
        ALTER TABLE public.alumnos ADD COLUMN perfil_completado BOOLEAN DEFAULT false;
    END IF;

    -- 4. Asegurar tipo DATE para fecha de nacimiento si existiera como text
    -- (Opcional, pero recomendado por legibilidad)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alumnos' AND column_name = 'fecha_nacimiento') THEN
        ALTER TABLE public.alumnos ALTER COLUMN fecha_nacimiento TYPE DATE USING fecha_nacimiento::DATE;
    END IF;

END $$;

-- Refrescar el caché de PostgREST para que reconozca las nuevas columnas inmediatamente
...
DECLARE
  v_plan_id uuid;
  v_rutina_id uuid;
  v_rutina record;    -- Cambiado a record
  v_ejercicio record; -- Cambiado a record
  v_rotacion record;  -- Cambiado a record
BEGIN
  IF p_profesor_id IS NOT NULL AND p_profesor_id != auth.uid() THEN
    RAISE EXCEPTION 'No autorizado';
  END IF;
  ...
  IF p_rotaciones IS NOT NULL AND jsonb_array_length(p_rotaciones) > 0 THEN
    FOR v_rotacion IN SELECT * FROM jsonb_array_elements(p_rotaciones) LOOP
      ...
      VALUES (
        v_plan_id,
        (v_rotacion.value->>'position')::int,
        ARRAY(SELECT jsonb_array_elements_text(v_rotacion.value->'applies_to_days')),
        v_rotacion.value->'cycles'
      );
    END LOOP;
  END IF;
