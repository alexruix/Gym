...
export function ImportStudentsModal({ isOpen, onOpenChange, onSuccess }: ImportStudentsModalProps) {
  const [file, setFile] = React.useState<File | null>(null);
  const [data, setData] = React.useState<ParsedStudent[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [processing, setProcessing] = React.useState(false);
  const copy = alumnosListCopy.list.import;
    ...
    reader.onload = (evt) => {
      try {
        ...
        const parsed = raw.map(row => {
          ...
          let tel = row.Telefono || row.telefono || row.WhatsApp || row.whatsapp;
  ...
  const handleImport = async () => {
    ...
    setLoading(true);
    try {
      const { data: res, error } = await actions.profesor.importStudents({ students: data as any });
      if (error) throw error;

      if (res.success) {
        toast.success(copy.success.replace("{count}", res.importedCount.toString()));
    ...
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col p-0 overflow-hidden rounded-t-[2.5rem] sm:rounded-3xl border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
        <DialogHeader className="p-6 pb-0">
          <DialogTitle className="text-xl sm:text-2xl font-black uppercase tracking-tight flex items-center gap-2">
          ...
          {!file ? (
            <label className="group relative flex flex-col items-center justify-center w-full h-48 sm:h-64 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl hover:bg-zinc-50 dark:hover:bg-zinc-900/50 hover:border-lime-400 transition-all cursor-pointer">
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-black">{copy.hint}</p>
              </div>
            ...
            <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-zinc-500">
                                {data.length} alumnos detectados
                            </p>
                        </div>
                    </div>
                ...
                <div className="space-y-3">
                    <h4 className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-zinc-400 px-1">{copy.preview}</h4>
                            ...
                            <table className="w-full text-sm min-w-[500px] sm:min-w-0">
                                <thead className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left font-black uppercase tracking-widest text-[9px] text-zinc-500">{copy.columns.email}</th>
                                        <th className="px-4 py-3 text-left font-black uppercase tracking-widest text-[9px] text-zinc-500">{copy.columns.phone}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                                    {data.slice(0, 5).map((row, i) => (
                                        <tr key={i} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-zinc-500 whitespace-nowrap">{row.email || '—'}</td>
                                            <td className="px-4 py-3 text-zinc-500 whitespace-nowrap">{row.telefono || '—'}</td>
                                        </tr>
                                    ))}
                                    {data.length > 5 && (
                                        <tr>
        ...
        <div className="p-4 sm:p-6 bg-zinc-50 dark:bg-zinc-900/50 border-t border-zinc-200 dark:border-zinc-800 flex flex-col sm:flex-row justify-end gap-3">
            ...
            <Button 
                ...
                className="w-full sm:w-auto rounded-xl bg-lime-400 hover:bg-lime-500 text-zinc-950 font-black uppercase tracking-widest text-[10px] h-12 px-6 shadow-lg shadow-lime-400/20 order-1 sm:order-2"
            >
                {loading || processing ? (
