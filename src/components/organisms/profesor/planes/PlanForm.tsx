          ...
          <div className="flex-1 p-6 lg:p-12 space-y-12 max-w-4xl mx-auto w-full pb-56 lg:pb-32">
            ...
            <header className="space-y-10 animate-in fade-in duration-700">
              <div className="space-y-4">
                 <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-ui-label">Nombre</h3>
                 <FormField
                 ...
                 <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-3">
                      <FormLabel className="industrial-label px-1">Duración total</FormLabel>
                      <div className="bg-ui-soft dark:bg-zinc-900 p-2 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex items-center gap-3">
                        <select 
            ...
            <section className="space-y-8">
               <div className="flex flex-col sm:flex-row gap-8 items-start sm:items-center justify-between">
                  ...
                  <div className="flex flex-wrap gap-3 shrink-0">
                     <Button type="button" variant="outline" size="xl" onClick={() => setIsBulkOpen(true)} className="px-6 h-16 rounded-[24px] border-zinc-100 dark:border-zinc-800">
                        <Copy className="w-5 h-5 mr-3" /> <span className="text-[10px] font-black uppercase">Duplicar día</span>
                     </Button>
                     {/* <Button type="button" variant="outline" size="xl" onClick={actions.saveDayAsBlock} className="px-6 h-16 rounded-[24px] border-zinc-100 dark:border-zinc-800 hover:border-lime-500 group/save">
                        <Box className="w-5 h-5 mr-3 text-zinc-300 group-hover/save:text-lime-500 transition-colors" /> <span className="text-[10px] font-black uppercase">Guardar bloque</span>
                     </Button> */}
                     <Button type="button" variant="industrial" size="xl" onClick={() => setIsAddElementOpen(true)} className="px-10 h-16 rounded-[24px] shadow-2xl shadow-lime-500/10">
                        <Plus className="w-5 h-5 mr-3" /> <span className="text-[10px] font-black uppercase">Añadir item</span>
                     </Button>
                  </div>
               </div>
               ...
               <div className="space-y-4 min-h-[300px]">
                  {currentExercises.length === 0 ? (
                    <div className="py-32 flex flex-col items-center text-center border-4 border-dashed border-zinc-100 dark:border-zinc-900 rounded-[40px] bg-ui-soft/20">
