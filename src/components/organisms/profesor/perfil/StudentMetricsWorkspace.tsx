         ...
         <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="space-y-4">
              <div className="flex items-center gap-3 text-fuchsia-400/80">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-[0.4em]">Consola Semanal</span>
              </div>
              <div className="space-y-1">
                <h3 className="text-4xl font-black tracking-tighter text-white uppercase italic leading-none">{metricsTab.title}</h3>
            ...
            <div className="flex flex-col gap-3">
               <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 ml-4">Semana Seleccionada</span>
            ...
            <div key={rutina.id} className="bg-white dark:bg-zinc-950/20 border border-zinc-100 dark:border-zinc-800 rounded-3xl overflow-hidden shadow-sm transition-all">
               <button onClick={() => toggleItem(rutina.id)} className="w-full flex items-center justify-between p-6">
                 <div className="flex items-center gap-6">
                    <span className={cn("w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm transition-all", isOpened ? "bg-zinc-950 text-white" : "bg-zinc-50 dark:bg-zinc-900 text-zinc-400")}>
                      {rutina.dia_numero}
                    </span>
                    <h4 className="font-black text-lg text-zinc-950 dark:text-white uppercase tracking-tighter">{rutina.nombre_dia || `Día ${rutina.dia_numero}`}</h4>
                 </div>
                        ...
                        <div key={ej.id} className="p-6 space-y-4 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/20 transition-all">
                           <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                              <div className="flex items-center gap-4 flex-1">
                                 ...
                                 <h5 className="font-black text-zinc-950 dark:text-zinc-50 uppercase tracking-tight text-lg leading-tight">{ej.biblioteca_ejercicios?.nombre}</h5>
                              </div>
                              <MetricConsole 
