      ...
      <Tooltip>
          ...
          >
            <div className="flex items-center gap-5">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-black text-lg text-zinc-950 dark:text-white uppercase tracking-tight truncate group-hover:text-lime-600 dark:group-hover:text-lime-400 transition-colors">
                  {block.nombre}
                </h4>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <div className="flex gap-1">
                    {(block.tags || []).map((tag: string) => (
                        <span key={tag} className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-500">
                          {tag}
                        </span>
                    ))}
                  </div>
        ...
        >
          <div className="space-y-4">
              ...
              {items.map((item: any, idx: number) => {
                ...
                return (
                  <li key={item.id} className="flex items-center gap-3">
                    ...
                    <span className="ml-auto text-[9px] font-black text-zinc-600 uppercase">
            ...
            {items.length === 0 && (
                <p className="text-[10px] font-black text-zinc-600 italic">No hay ejercicios en este bloque.</p>
            )}
          </div>
        </TooltipContent>
      </Tooltip>
