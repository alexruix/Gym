...
export function EntitySelectorDialog<T extends BaseEntity>({
    ...
    const filteredItems = useMemo(() => items.filter(filterFn), [items, search, activeTags]);

    const filteredSections = useMemo(() => {
        if (!sections) return null;
        return sections.map(s => ({
            ...s,
            items: s.items.filter(filterFn)
        })).filter(s => s.items.length > 0);
    }, [sections, search, activeTags]);

    const hasResults = sections ? (filteredSections?.length || 0) > 0 : filteredItems.length > 0;
    ...
    const handleSelectAll = () => {
        const allIds = sections
            ? filteredSections?.flatMap(s => s.items.map(i => i.id)) || []
            ...
            <DialogContent className="max-w-md p-0 overflow-hidden border-zinc-200 dark:border-zinc-800 rounded-[32px] bg-white dark:bg-zinc-950 shadow-2xl">
                <DialogHeader className="p-6 pb-4 border-b border-zinc-100 dark:border-zinc-900 bg-zinc-50/50 dark:bg-zinc-900/10">
                    <div className="flex items-center justify-between gap-3">
                        ...
                        {/* {selectedIds.size > 0 && multiple && (
                            <div className="bg-lime-500 text-zinc-950 px-3 py-1 rounded-full text-[10px] font-bold animate-in zoom-in duration-300">
                ...
                <div className="p-4 border-b border-zinc-100 dark:border-zinc-900 relative group bg-white dark:bg-zinc-950">
                    ...
                    {/* {multiple && hasResults && (
                        <div className="flex items-center gap-4 mt-3 px-2">
                           ...
                           <button onClick={handleClearSelection} className="text-[9px] font-bold uppercase tracking-widest text-zinc-400 hover:text-red-500 transition-colors">Limpiar selección</button>
                        </div>
                    )} */}
                ...
                <div className="max-h-[450px] overflow-y-auto no-scrollbar">
                    {children ? children : isLoading ? (
                        ...
                        <div className="py-20 flex flex-col items-center gap-4 text-center px-12">
                            ...
                            {onCreateNew && (
                                <button onClick={onCreateNew} className="text-lime-500 px-1 flex items-center font-bold text-[10px] uppercase tracking-widest h-10">
                        ...
                        <div className="space-y-6 pb-6">
                            {filteredSections?.map((section, sIdx) => (
                                <div key={section.title + sIdx} className="space-y-1">
                                    <div className="px-6 py-2 bg-zinc-50/50 dark:bg-zinc-900/20 border-y border-zinc-100 dark:border-zinc-900/50 sticky top-0 z-10 backdrop-blur-sm">
