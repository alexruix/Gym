...
export function PlanCard({ plan, onDelete, onDuplicate }: PlanCardProps) {
  return (
    <Card className="group relative bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 overflow-hidden rounded-3xl hover:shadow-2xl hover:shadow-lime-500/5 transition-all duration-500 flex flex-col h-full border-b-[6px] border-b-zinc-100 dark:border-b-zinc-900 hover:border-b-lime-500/20">
      ...
      {/* Media / Visual Area */}
      <div className="aspect-[16/10] w-full bg-zinc-50 dark:bg-zinc-900 flex items-center justify-center relative overflow-hidden shrink-0 border-b border-zinc-100 dark:border-zinc-800/50 cursor-pointer" onClick={() => window.location.href = `/profesor/planes/${plan.id}`}>
      ...
      <div className="p-6 space-y-4 flex-1 flex flex-col">
        <div className="flex-1 space-y-2">
            <div className="flex items-start justify-between gap-4">
                <h3 className="font-black text-xl text-zinc-950 dark:text-zinc-50 leading-tight group-hover:text-lime-600 dark:group-hover:text-lime-400 transition-colors line-clamp-2 uppercase tracking-tight cursor-pointer" onClick={() => window.location.href = `/profesor/planes/${plan.id}`}>
                    {plan.name}
                </h3>
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-52 p-2 rounded-2xl border-zinc-200 dark:border-zinc-800 shadow-2xl bg-white dark:bg-zinc-950 font-bold z-50">
                        <DropdownMenuItem 
                            ...
                            className="flex items-center gap-3 p-3 rounded-xl cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors"
                        >
