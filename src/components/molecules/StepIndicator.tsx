  ...
  return (
    <div className="w-full py-4">
      <div className="flex items-center justify-between relative">
        ...
        <div
          className="absolute left-0 top-1/2 -translate-y-1/2 h-0.5 bg-lime-500 z-0 transition-all duration-500"
          ...
          return (
            <div key={idx} className="flex flex-col items-center relative z-10 bg-white px-2">
              <div className={cn(
                ...
                isCompleted ? "bg-lime-500 border-lime-400 text-zinc-950" :
                  isActive ? "bg-zinc-950 border-zinc-950 text-white scale-110 shadow-lg" :
                    "bg-white border-zinc-200 text-zinc-400"
              )}>
                {isCompleted ? <Check className="w-5 h-5" /> : <span className="text-xs font-bold">{stepNum}</span>}
              </div>
              <span className={cn(
                "absolute -bottom-6 text-[8px] font-bold uppercase tracking-widest whitespace-nowrap",
